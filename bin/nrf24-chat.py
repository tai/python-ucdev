...
def main(ctx):
    ...
    # TODO: Try doing multi-node chat with promisc mode
    #rf.SETUP_AW.AW = 1
    #rf.CONFIG.EN_CRC = 0

    # send/recv loop
    while True:
        fd = select.select([sys.stdin], [], [], 0.0)

        if fd[0]:
            input = fd[0][0].readline().strip()
            log.info("send: %s" % input)
            rf.set_mode(DIR_SEND)
            rf.send(input.ljust(32)[:32])
            rf.set_mode(DIR_RECV)
...
if __name__ == '__main__' and '__file__' in globals():
    ...
    if len(ctx.opt.rx) == 0:
        # NOTE:
        # In ESB mode, TX_ADDR and RX_ADDR_P0 must be same for Auto-ACK
